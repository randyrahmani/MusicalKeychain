...
static void drawEndingText(PGM_P text, int16_t x, int16_t y) {
  ...
  while ((codePoint = pgm_read_byte(text++)) != 0) {
    ...
    display.drawBitmap(x, y, glyph->bitmap, ENDING_GLYPH_WIDTH,
                       ENDING_GLYPH_HEIGHT, SH110X_WHITE);
    ...
    if (codePoint == ' ') {
      ...
      if (latinGlyph != nullptr) {
        display.drawBitmap(x, y, latinGlyph->bitmap, LATIN_GLYPH_WIDTH,
                           LATIN_GLYPH_HEIGHT, SH110X_WHITE);
      }
    } else {
      ...
      if (glyph != nullptr) {
        display.drawBitmap(x, y, glyph->bitmap, HANGUL_GLYPH_WIDTH,
                           HANGUL_GLYPH_HEIGHT, SH110X_WHITE);
      } else {
        display.drawRect(x + 1, y + 1, HANGUL_GLYPH_WIDTH - 2,
                         HANGUL_GLYPH_HEIGHT - 2, SH110X_WHITE);
      }
    }
    x += advance;
  }
...
static void drawHeart(int16_t centerX, int16_t centerY, uint8_t size) {
  ...
  display.fillCircle(centerX - radius, lobeY, radius, SH110X_WHITE);
  display.fillCircle(centerX + radius, lobeY, radius, SH110X_WHITE);
  display.fillTriangle(centerX - 2 * radius, lobeY,
                       centerX + 2 * radius, lobeY,
                       centerX, top + size, SH110X_WHITE);
}
